...
4. SQLite 只保存当前已接受状态，不长期保存旧 generation 的完整图或操作历史。核心关系采用关系表：Target、位置、直接安装要求、精确来源、Package、依赖边、投影和 Detached Override baseline。小型 transform 参数可使用受约束 JSON。
5. dependency observation 可以存在同一数据库中，但属于可删除重建的非权威观察状态，不参与 Target generation。
6. 一次完整状态变化先完成来源/解析/Store/Target preflight 和用户接受，再执行 Target 变化，最后用一个 SQLite transaction 原子替换该 Target 的完整当前状态并递增 generation；`.skiloom-state` 最后同步 generation。
7. 因 SQLite 与 Target filesystem 不能形成跨资源 ACID transaction，v0 允许一组临时 `pending_operations` / `pending_projection_actions` 记录，用于进程中断后安全识别本次 Skiloom 创建或修改的路径并恢复到最后已提交状态。正常完成后立即删除；它不是安装历史或第二份 Lock。
